...
function Board({ collectionName }: { collectionName: string }) {
  ...
  const fetchPosts = async (keyword = "") => {
    ...
    if (keyword) {
      ...
      // 검색어가 없으면
      firstPageQuery = query(collection(db, collectionName), orderBy("createAt", "desc"), limit(postsPerPage));
    }
  ...
  const fetchTotalPosts = async () => {
    ...
    setTotalPosts(querySnapshot.size); //총게시글의 숫자를 가져옴
  };
  ...
  const goToPage = async (pageNumber: number) => {
    ...
    if (pageNumber === 1) {
      pageQuery = query(collection(db, collectionName), orderBy("createAt", "desc"), limit(postsPerPage));
    } else {
  ...
  async function getStartAtDocumentForPage(pageNumber: number, postsPerPage: number) {
    ...
    const pageQuery = query(collection(db, collectionName), orderBy("createAt", "desc"), limit(skipDocs + 1));
    const querySnapshot = await getDocs(pageQuery);
  ...
  const write = () => {
    ...
    collectionName === "contents" ? navigate("/normalWrite") : navigate("/spoWrite");
  };

  return (
    <>
      <div className="board flex justify-center ">
        <div className="bg-oklch pb-4 px-4 rounded-md w-2/3 glass mt-10">
          <div className="flex justify-between w-full pt-6 ">
            <p className="title ml-3 text-gray-300 text-lg">
              {collectionName === "spoils" ? "스포 게시판" : "일반 게시판"}
            </p>
